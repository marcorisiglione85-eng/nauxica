...
Five service types are active at Sicily launch. Every assignment covers exactly one service type. A property may have multiple assignments — one per service type, or multiple assignments for the same type with different priority ranks.
...
| `MAINTENANCE` | Repairs, technical fixes, installations, inspections; at MVP also covers pool and garden maintenance requests | ServiceRequest from guest or homeowner; scheduled inspection; scheduled maintenance visit |
| `TRANSFERS` | Airport/port pickups and dropoffs, inter-city transfers | Guest arrival/departure; on-request via AI concierge |
| `EXPERIENCES` | Tours, boat trips, cooking classes, wine tastings, cultural visits | On-request via AI concierge; proactive recommendation |

### Post-MVP Service Subtypes

The following codes are not active as `service_type` values at Sicily launch. At MVP, pool and garden maintenance jobs are created as `MAINTENANCE` requests. In-person concierge is not offered at MVP. These codes are reserved for future activation as service subtypes when separately launched.

| Subtype Code | Description | Status |
|---|---|---|
| `POOL_MAINTENANCE` | Pool cleaning, chemical balance, equipment checks | Post-MVP subtype |
| `GARDEN_MAINTENANCE` | Grounds, irrigation, landscaping | Post-MVP subtype |
| `CONCIERGE_IN_PERSON` | On-site personal concierge (premium tier only) | Post-MVP subtype |
| `INSPECTION` | Pre-activation inspection, inventory check, damage assessment | Post-MVP subtype |
...
| `service_type` | Enum | internal | Yes | One of the 5 MVP service_type values: `CLEANING`, `MAINTENANCE`, `LAUNDRY`, `TRANSFERS`, `EXPERIENCES`. Post-MVP service subtypes use a separate `service_subtype` field. |
...
| `schedule_type` | Enum | internal | No | `on_demand` / `scheduled_recurring` / `both`. For cleaning and maintenance, `on_demand` is standard. For periodic scheduled maintenance, `scheduled_recurring` is typical. |
...
> Response windows for `POOL_MAINTENANCE` and `GARDEN_MAINTENANCE` will be defined when those subtypes are activated post-MVP.
...
| TRANSFERS | Pickup/dropoff address (from reservation, not property), guest name, guest count, flight/ferry details from reservation |
| EXPERIENCES | Relevant experience description, booking reference, guest count |
| POOL_MAINTENANCE *(Post-MVP subtype)* | `pool_type`, `pool_instructions`, `pool_heated`, pool maintenance notes from `property_quirks_for_partners` |
| GARDEN_MAINTENANCE *(Post-MVP subtype)* | `garden_instructions`, `garden_available` |
| INSPECTION *(Post-MVP subtype)* | Full PARTNER brief + access to all Category H fields |
